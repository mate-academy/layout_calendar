...
- [DEMO LINK](https://Tolik-code.github.io/layout_calendar/)
- [TEST REPORT LINK](https://Tolik-code.github.io/layout_calendar/report/html_report/)
...
- Implement `start-day` modifier for the `calendar` with `mon`, `tue`, `wed`, `thu`, `fri`, `sat` and `sun` values
  ...
  - The calendar-month should start at the correct column (Monday is the 1st, Friday is the 5th)
  - You can just add correct `margin-left` for the first day
- Add a modifier `calendar-month-length` for the `calendar` with values 28, 29, 30 and 31 (use `@for`)
