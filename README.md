...
- [DEMO LINK](https://Mykhailyk-Vadym.github.io/layout_calendar/)
- [TEST REPORT LINK](https://Mykhailyk-Vadym.github.io/layout_calendar/report/html_report/)
...
- Create a markup for the calendar block with 31 dates inside
  - DON'T add numbers in HTML (you will do it using CSS)
- Each date is a grey (`#eee`) `100px` square (including 1px black border)
  - Add a number (`Arial 30px`) in the center of each date using `::before` and [@for](https://sass-lang.com/documentation/at-rules/control/for)
...
- Implement `start-date` modifier for the `calendar` with `mon`, `tue`, `wed`, `thu`, `fri`, `sat` and `sun` values
  - Use [@each](https://sass-lang.com/documentation/at-rules/control/each) to create all the modifiers
  - The month should start at the correct column (Mondate is the 1st, Fridate is the 5th)
  - You can just add correct `margin-left` for the first date
- Add a modifier `month-length` for the `calendar` with values 28, 29, 30 and 31 (use `@for`)
  - It sets the last date to show (use [nth-child](https://css-tricks.com/how-nth-child-works/))
